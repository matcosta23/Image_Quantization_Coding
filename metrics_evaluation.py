...
class Distortion_Evaluation():
    ...
    def plot_mse_scatter(self, quantizer_id, img_name, plot_file_path=None):
        ...
        sorted_results = self.results[np.argsort(self.results[:, 2])]
        # Get values in the hull.
        convex_hull_indexes = [True] + list(map(lambda idx: np.all(sorted_results[idx, 1] < sorted_results[:idx, 1]), range(1, len(sorted_results))))
        ...
        scatters = []
        for mse, bpp in sorted_results[:, 1:3]:
    ...
    def save_results(self, csv_path_to_save):
        df = pd.DataFrame(self.results, columns=['PSNR', 'MSE', 'BPP', 'N', 'M'])
        df.to_csv(csv_path_to_save)
        return        